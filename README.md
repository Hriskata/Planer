...
- Изглед по ден, седмица (часова мрежа 00–23ч) и месец, навигация напред/назад/днес; клик на ден в месечния изглед превключва към дневния
- Съботите и неделите са оцветени в светлосиво в седмичния и месечния изглед
- Създаване, редакция, отмятане като завършена, изтриване и **копиране** на задача (заглавие, дата, час на стъпки от 15 мин, бележки, цвят, "споделена")
- Цветово кодиране на задачи (зелено/жълто/червено) — виден в дневния списък (лента отляво), седмичния и месечния изглед
- Drag-and-drop на задачи в седмичния изглед — местене на друг ден/час чрез влачене (Pointer Events, работи и с пръст на телефон)
- Офлайн resilience: последно заредените задачи се кешират в `localStorage`; при загубена връзка се показват кешираните данни с банер "Няма връзка". Промени (създаване/редакция/изтриване/местене) офлайн връщат грешка вместо да се опашкуват — пълна двупосочна синхронизация е извън обхвата на MVP.
