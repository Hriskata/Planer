...
backend/src/
  ...
  schema.sql        пълна схема (users, tasks, push_subscriptions, calendar_shares)
  ...
  routes/           auth, tasks, account, sharing, push, uploads

frontend/src/
  App.svelte        LandingPage → LoginView → MainView, по auth състояние
  lib/MainView.svelte   основната обвивка (header, филтри, изгледи, FAB, TaskForm modal)
  ...
  lib/CalendarSwitcher.svelte   header dropdown за превключване между календари
...
- **Две различни sharing механики съжителстват**: `tasks.shared` (булево, видимо за ВСИЧКИ потребители в инстанцията) vs. `calendar_shares` (targeted, read-only, по имейл, целия календар на притежателя).
...
- **Текущ статус (последно обновено при създаването на този файл)**: функцията за споделяне на календар (calendar_shares, CalendarSwitcher, read-only TaskForm режим) е завършена, тествана и прегледана (code-review pass, 5 находки оправени) — **чака "Билдни"**. Заедно с нея чака и по-ранна недовършена промяна: бутон "← Начало" на login екрана за връщане към лендинг страницата. И двете са некомитнати в git към момента на писане.
