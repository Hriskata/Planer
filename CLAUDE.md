...
- **Текущ статус (обновено 2026-08-14)**: нова функционалност "Асет библиотека" (`library_assets` таблица, `routes/library.js`, `LibraryPage.svelte`, нов бутон "Библиотеки" до логото в header-а на MainView) — брандинг материали по клиент (Лого/Шрифт/Снимка/Текст/Друго), споделяни през същия `calendar_shares` механизъм, но с различен permission модел (виж т.4) — тествана end-to-end, чака "Билдни".
